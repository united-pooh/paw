...
export interface WorkbenchShellProps {
  ...
  /** 分叉当前会话（复制上下文创建新会话并切换过去） */
  onFork?: (workspaceID: string, sessionID: string) => Promise<void>;
...
export function WorkbenchShell({ workspaces, sessions, snapshot, parts, tools, interactions, onSelectWorkspace, onSelectSession, onOpenWorkspace, onCreateSession, onSubmit, onSteer, onQueue, onCancel, onFork, onAnswer, onDecide }: WorkbenchShellProps) {
  ...
  // 发送信号：用户每次提交消息（正常/steer/排队）递增一次，通知对话区强制回到底部。
  // ConversationView 以会话为 key 重挂载，切会话后新实例以当前信号为基线，不会误触发。
  const [sendSignal, setSendSignal] = useState(0);
  const bumpSendSignal = () => setSendSignal((value) => value + 1);
  ...
  useLayoutEffect(() => {
    ...
    const measure = () => {
      const clearance = dock.getBoundingClientRect().height - parseFloat(getComputedStyle(dock).paddingTop || '0');
      workspace.style.setProperty('--dock-clearance', `${Math.max(0, clearance)}px`);
      const viewport = workspace.querySelector<HTMLElement>('.conversation-view');
      workspace.style.setProperty('--conversation-scrollbar-width', `${viewport ? viewport.offsetWidth - viewport.clientWidth : 0}px`);
    };
  ...
  return <main className={`workbench-shell${panelCollapsed ? ' panel-collapsed' : ''}`}>
    ...
    <section className="main-workspace" ref={workspaceRef}>
      ...
      <div className="dock" ref={dockRef}>
        ...
        {selectedWorkspaceID && selectedSessionID && onSubmit ? <Composer key={`${selectedWorkspaceID}:${selectedSessionID}`} workspaceID={selectedWorkspaceID} sessionID={selectedSessionID} activeTurnID={activeSnapshot?.active_turn_id} queueCount={activeSnapshot?.queue?.length ?? 0}
          onSubmit={(text, commandID) => { bumpSendSignal(); return onSubmit(selectedWorkspaceID, selectedSessionID, text, commandID); }}
          onSteer={onSteer ? (text, commandID, turnID) => { bumpSendSignal(); return onSteer(selectedWorkspaceID, selectedSessionID, text, commandID, turnID); } : undefined}
          onQueue={onQueue ? (text, commandID, turnID) => { bumpSendSignal(); return onQueue(selectedWorkspaceID, selectedSessionID, text, commandID, turnID); } : undefined}
