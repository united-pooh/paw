...
func TestSelectToolDisplayTargetAndResultSummary(t *testing.T) {
	...
	if got, ok := selectToolCallTarget("Select", input); !ok || got != "" {
		t.Fatalf("target = %q, %v", got, ok)
	}
	if got, ok := selectToolResultTarget("Select", "ok", `{"cancelled":false,"selected_options":[{"id":"prod","label":"Production"}]}`); !ok || got != "selected 1 option" {
		t.Fatalf("summary = %q, %v", got, ok)
	}
	if got, ok := selectToolResultTarget("Select", "ok", `{"cancelled":true,"selected_options":[]}`); !ok || got != "cancelled" {
