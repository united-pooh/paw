// Package textareax is a fork of github.com/charmbracelet/bubbles/textarea
// (v1.0.0) with a linear-time wrap implementation. Upstream's wrap() re-measures
// the full line/word width for every rune, making per-keystroke re-wrapping of
// the current line O(n²). This fork keeps behavior identical while tracking
// widths incrementally. Re-sync upstream changes carefully.
package textareax
...
import (
	...
	"paw/internal/ui/bubble/textareax/memoization"
...
// wrap performs word wrapping over runes. Behavior matches upstream
// bubbles/textarea exactly, but line/word widths are tracked incrementally:
// lineWidth/wordWidth are running upper bounds of uniseg.StringWidth
// (Σ RuneWidth ≥ grapheme-aware width), so the exact measurement only runs
// near wrap boundaries instead of once per rune. Upstream re-measured the full
// line/word for every rune, making per-keystroke re-wrap of the current line
// O(n²).
func wrap(runes []rune, width int) [][]rune {
	var (
		...
		row       int
		spaces    int
		lineWidth int // Σ RuneWidth over lines[row]
		wordWidth int // Σ RuneWidth over word
	)
	...
	for _, r := range runes {
		if unicode.IsSpace(r) {
			...
			word = append(word, r)
			wordWidth += rw.RuneWidth(r)
		}

		if spaces > 0 { //nolint:nestif
			if lineWidth+wordWidth+spaces > width &&
				uniseg.StringWidth(string(lines[row]))+uniseg.StringWidth(string(word))+spaces > width {
				row++
			...
			if wordWidth+lastCharLen > width &&
				uniseg.StringWidth(string(word))+lastCharLen > width {
