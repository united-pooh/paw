...
export function SessionList({ sessions, selected, onSelect, onCreate }: { sessions: SessionSummary[]; selected?: string; onSelect: (id: string) => void; onCreate?: () => void }) {
  return <section className="session-list" aria-label="会话列表">
    <div className="session-list-header"><span>会话</span><button type="button" disabled={!onCreate} onClick={onCreate}>＋</button></div>
